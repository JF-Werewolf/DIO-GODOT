...
func checkTarget():
	if parent.get_node("targetArea").get_overlapping_bodies().size() >1:
		...
		for body in bodies:
			...
			if(randi_range(0,distance *0.7) == int(distance/2)):
				print("DISTANCE:", distance, "   DIS CHANCE: ", int(distance/60))
...
func processPhysics(delta):	
	...
	if action == 3:
		...
		targetSpeed = parent.WalkSpeed
		#newDir = parent.getAnimDir(parent.angle)
		
		
		if((positionTarget - parent.global_position).length() < 15):
			action = 0
